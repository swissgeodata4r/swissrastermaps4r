...
If multiple rasters are returned, the output is a `list()` of rasters with a length corresponding to the number of featuers (i.e. rows) in the input features.
...
tm_shape(rastermap[[1]]) + 
  tm_rgb() + 
  tm_shape(gemeinden_top_centroid[1,]) + 
  tm_dots(size = "EINWOHNERZ")


tm_shape(rastermap[[2]]) + 
  tm_rgb() + 
  tm_shape(gemeinden_top_centroid[2,]) + 
  tm_dots(size = "EINWOHNERZ")
```
